 ...
 *
 * Compile with:
 *   gcc -o tunecurses tunecurses.c -lncurses -lpthread
 *
 * Run with:
 *   ./tunecurses [starting_directory]
 */
...
/* Returns the path to the state file (e.g., ~/.tunecurses_state) */
static char *get_state_file_path(void) {
    static char path[PATH_MAX];
    char *home = getenv("HOME");
    if (!home) return NULL;
    snprintf(path, sizeof(path), "%s/.tunecurses_state", home);
    return path;
}
...
void save_state(void) {
    ...
    fprintf(f, "# TuneCurses State File\n");
...
void draw(void) {
    ...
    snprintf(header_left, sizeof(header_left), "TuneCurses %s[%s theme] %s",
